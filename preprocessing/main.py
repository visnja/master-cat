...
def get_spacy_matcher():
    ...
    if not SPACY_MATCHER:
        ...
        matcher.add('AUD', [
            ...
            [{'LOWER': 'aud'}],
            [{'LOWER': 'aussie'}],
            [{"LOWER": 'the', 'OP': '?'}, {"LOWER": 'australian'},
                {'LOWER': {'REGEX': 'dollar[s]?'}}]
        ])
        matcher.add('USD', [
        ...
        matcher.add('GBP', [
            ...
            [{'LOWER': 'gbp'}],
            [{'LOWER': 'the', 'OP': '?'}, {'LOWER': 'british',
                                           'OP': '?'}, {'LOWER': {'REGEX': 'pound[s]?'}}],
        ...
        matcher.add('CNY', [
            ...
            [{'LOWER': 'the', 'OP': '?'}, {'LOWER': 'chinese'}, {
                'LOWER': 'yuan'}, {"IS_PUNCT": True, 'OP': '?'}],
            [{'LOWER': {'REGEX': 'yuan[s]?'}}]
        ])
        matcher.add('TWD', [
        ...
        matcher.add('INDEX', [
            ...
            [{'LOWER': 'djia'}],
            [{'LOWER': 'dow'}, {'LOWER': 'jones'}, {
                'LOWER': 'industrial'}, {'LOWER': 'average'}],
            [{'LOWER': 'ftse'}, {"ORTH": '100', 'OP': '?'}],
            [{'LOWER': 'financial'}, {'LOWER': 'times'}, {
                'LOWER': 'stock'}, {'LOWER': 'exchange'}],
            [{'LOWER': 'footsie'}],
            [{'LOWER': 'dax'}],
            [{'LOWER': 'dax'}, {"ORTH": '40', 'OP': '?'}],
            [{'LOWER': 'german'}, {'LOWER': 'stock'}, {'LOWER': 'index'}],
            [{'LOWER': 'nikkei'}, {"ORTH": '225', 'OP': '?'},
                {'LOWER': 'index', 'OP': '?'}],
            ...
            [{'LOWER': {'REGEX': 'fed[eral]?'}}, {
                'LOWER': 'reserve'}, {'LOWER': 'bank'}],
...
if __name__ == '__main__':
    ...
    df = df.drop("contentType", "visited", "icon", 'classes_body', 'classes_summary',
                 'classes_target', 'classes_title', 'source', 'alternateImageUrl')
    ...
    sql = sql.withColumn("target", join_tokens(
        col("classes"), col('classes_title'), col('classes_body')))
    sql.withColumn("removed_stopwords", tokenize_and_clean(
        col("body"))).select("removed_stopwords").show()

    sql.write.mode("overwrite").parquet(
        "hdfs://namenode:9000/user/root/labeled.parquet")
